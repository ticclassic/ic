...
*Setup instructions for a gitian build of Brain using a Debian VM or physical system.*

Gitian is the deterministic build process that is used to build the Brain
...
Clone the git repositories for brain and gitian and then checkout the brain version that you want to build.
...
Follow the instructions in [doc/release-process.md](release-process.md) in the brain repository
...
To build Brain (for Linux, OSX and Windows) just follow the steps under 'perform
gitian builds' in [doc/release-process.md](release-process.md) in the brain repository.
...
```bash
    Initialized empty Git repository in /home/debian/gitian-builder/inputs/brain/.git/
...
For example:
```bash
URL=https://github.com/crowning-/brain.git
COMMIT=b616fb8ef0d49a919b72b0388b091aaec5849b96
./bin/gbuild --commit brain=${COMMIT} --url brain=${URL} ../brain/contrib/gitian-descriptors/gitian-linux.yml
./bin/gbuild --commit brain=${COMMIT} --url brain=${URL} ../brain/contrib/gitian-descriptors/gitian-win.yml
./bin/gbuild --commit brain=${COMMIT} --url brain=${URL} ../brain/contrib/gitian-descriptors/gitian-osx.yml
```
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/brain-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/brain-build.assert
    gpg --detach-sign ${VERSION}-osx/${SIGNER}/brain-build.assert
```
...
[brain/gitian.sigs](https://github.com/axelxod/gitian.sigs/) repository, or if that's not possible to create a pull
request.
