...
Braincoin Core 0.11.0 is forked off the Bitcoin Core 0.9.3. The old braincoin
...
with the introduced changes to the Braincoin Core 0.11.0.


Braincoin Core:

- Rebrand to `Braincoin Core`
...
- Defined BIP32 (HD) address versions to start with `drkp`/`drkv` (`DRKP`/`DRKV`
  for testnet)
- Adapted BIP44 coin type `5` for Braincoin (0x80000005) as defined in SLIP-0044
...
- Add separate braincoin-cli client
...
- Add '-disablewallet' mode to let braincoind run entirely without wallet (when
  built with wallet)
- Update default '-rpcsslciphers' to include TLSv1.2
- make '-logtimestamps' default on and rework help-message
- RPC client option: '-rpcwait', to wait for server start
- Remove '-logtodebugger'
- Allow `-noserver` with braincoind
...
- Fix "Start braincoin on system login"
...
- Show and store message of normal braincoin:URI
- Fix richtext detection hang issue on very old Qt versions
- OS X: Make use of the 10.8+ user notification center to display Growl-like
  notifications
- OS X: Added NSHighResolutionCapable flag to Info.plist for better font
  rendering on Retina displays.
- OS X: Fix braincoin-qt startup crash when clicking dock icon
- Linux: Fix Gnome braincoin: URI handler
