...
- saction for Ramsend. These are special transactions that require a signature that only the masternodes can create.
- Ramsend now has no fees to track what-so-ever, all that will ever be in Ramsend transactions are Ramsend denominations.
...
- Split up ramsend.cpp into masternode.cpp, activemasternode.cpp and instantx.cpp
- Added modular ProcessMessages for Ramsend, Masternodes and InstantX
...
- Fixed all sorts of issues with collateral and Ramsend.
...
- Ramsend tested with 2.5 DRK successfully and I've set that as the minimum
...
- Ramsend UI is disabled for masternodes now and titlebar says "[masternode]"
...
- Updated wording for ramsend messages
- Removed disable ramsend checkbox from config screen
- Added "-enableramsend" cli option
...
- Ramsend should anonymize very close to the intended amount now
...
- This update deals with freezing, slow wallets, slow load times and the
  ...
  Ramsend process has been reserve keys for every attempt! To rectify this
...
- Easy to read ramsend progress bar (Mouse over for very detailed info)
...
- Majorly improved the way ramsend participants are paired together. It should
  be super fast now.
...
- Ramsend now shows address instead of "n/a"
...
- Added GUI configuration for Ramsend Rounds, Enable Ramsend and Anonymize
  amount of DRK
...
- Ramsend limited to 5000DRK per wallet.dat. Client will warn about this the
  first time it's opened, then disable ramsend from then on.
...
- Found race condition with new blocks and clearing ramsend entries that was
...
- Transactions now shows ramsend transaction types for easier understanding
  of what's going on:
...
- Fixed the money destruction bug, it was caused by "ramsend denominate 8000".
  I missed a reference and the client passed an empty address to
  SendMoneyToDestination. rcp ramsend source: http://pastebin.com/r14piKuq
...
- Added Ramsend detection to the UI
- fixed senttoaddress, it will use all inputs when ramsend is disabled now.
  Otherwise it will ONLY use non-denom.
- "ramsend addr amount" now returns the hash of the transaction it creates
...
- Removed "ramsend denominate", ramsend now will figure out the most it can
  denominate. Use "ramsend auto" instead.
...
- QT GUI will now ask to unlock the wallet when it detects Ramsend wants to do
  something and lock it when it's done again.
- Ramsend is turned off by default in the daemon now. In most cases daemons
  won't want to run with anonymity (pools, exchanges, etc), if a user does they
  can override the default setting with -enabledaemonramsend=true
- Fees per round of Ramsend are 0.001DRK or $0.00538 at current prices. This
...
- RPC calls are changed a bit:
    ...
    ramsend denominate 100 - Will denominate 100DRK
    ramsend auto - Will run AutoDenominate
    ramsend Xaddr 100 - Will send 100 denominated DRK to Xaddr
...
- Fixed ramsend+ rounds function, it was returning bad data in some cases
  causing ramsend inputs to never stop being sent through ramsend.
...
- Added a smart input splitting method. Place 1000+DRK into a brand new wallet
  and it will be split into many inputs compatible with Ramsend
...
- New column "Ramsend Rounds" in coincontrol to show how secure a given input is
- Fixed a few issues causing ramsend to fail. We should see many more ramsends
...
- Redid denominations to 1, 10, 100, and 500. Maybe this is too simple, but it
  ...
  again through ramsend for the next transactions. We'll see how it works.
...
- Improved RamsendInput add entry verification. Masternodes will now reject
