    ...
    fFeeMinimized(true)
{
    ...
    connect(ui->checkUseRamsend, SIGNAL(stateChanged ( int )), this, SLOT(updateDisplayUnit()));
...
void SendCoinsDialog::on_sendButton_clicked()
{
    ...
    if(ui->checkUseRamsend->isChecked()) {
        ...
        strFee = QString(tr(
            "(ramsend requires this amount to be rounded up to the nearest %1)."
...
void SendCoinsDialog::coinControlChangeEdited(const QString& text)
{
    if (model && model->getAddressTableModel())
    {
        ...
        {
            ui->labelCoinControlChangeLabel->setText(tr("Warning: Invalid Braincoin address"));
        }
        else // Valid address
        {
            CPubKey pubkey;
